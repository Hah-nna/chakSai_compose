...
fun CommentUI(itemData: Comment, img: Int) {
    Column(
        ...
        Row (
            ...
            ProfileSmall(img)
            Text(itemData.userInfo.nickName, modifier = Modifier.padding(horizontal = AppTheme.size.small))
            Text(itemData.createdAt, style = AppTheme.typography.labelSmall, color = Color.DarkGray)
        }
